...
- `GET /api/image-queue/me`，返回当前 Bearer Token 的队列状态。可选查询参数 `request_id` 会带回那一条请求的 `status`、`position`、`ahead`、`started_at`、`finished_at`、耗时和错误信息。接口同时返回当前 Bearer Token 的等待数、运行数、活动数，全局等待数、运行数、活动数，以及 60 次/60 秒启动限制的窗口状态。
