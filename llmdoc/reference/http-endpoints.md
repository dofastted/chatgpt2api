...
- `POST /api/donations/accounts`，接收 `tokens: string[]`，把账户按“捐赠”分类入池，然后刷新账号信息，位置在 `services/api.py:283`。
...
- `POST /api/accounts`，接收 `tokens: string[]`，先新增再刷新账号信息，默认按“普通”分类入池，位置在 `services/api.py:265`。
...
- `POST /api/accounts/update`，接收 `access_token` 和部分更新字段；更新字段现在可包含 `category`，位置在 `services/api.py:360`。
...
- 登录、会话、额度、账号列表、新增、捐赠新增、刷新、更新都在 `web/src/lib/api.ts:99` 到 `web/src/lib/api.ts:178`。
