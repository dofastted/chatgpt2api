...
- `POST /api/donations/accounts`，接收 `tokens: string[]` 或 `accounts: object[]`，把账户按“捐赠”分类入池；账号不会自动访问上游刷新信息，需要管理员后续手动刷新，位置在 `services/api.py:635`。
...
- `POST /api/accounts`，接收 `tokens: string[]` 或 `accounts: object[]`，默认按“普通”分类入池；账号不会自动访问上游刷新信息，需要管理员后续手动刷新，位置在 `services/api.py:607`。
