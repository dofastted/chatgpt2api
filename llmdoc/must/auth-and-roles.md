...
- 用户 key 走独立服务与读写，数据主存储在 SQLite，数据结构现在除了 `quota` 和状态，还包含 `gpt-image-2=2`、`gpt-image-2-2K=2`、`gpt-image-2-4K=8` 三个默认模型单价，服务入口在 `services/user_key_service.py`。
- 旧 `data/user_keys.json` 只在 SQLite 对应文档为空时导入。运行时修改不会再反向写回该 JSON。
...
- 顶部导航继续只给管理员显示“号池管理”，账号页内包含“数据管理”tab，位置在 `web/src/components/top-nav.tsx:13` 和 `web/src/app/accounts/page.tsx`。
