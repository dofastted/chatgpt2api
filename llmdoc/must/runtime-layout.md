...
- 当前项目还有一条仓库外入口 `img.fkcodex.com`。主线路已经迁到 VPS：远端宿主机 Nginx 当前反代 `127.0.0.1:3306`，live 容器是 `chatgpt2api-green-7188696`（镜像 `chatgpt2api:7188696`，含账号统计口径修复 + 恢复账号池管理表格，整 HEAD 构建）；回滚链：`chatgpt2api-green-92129ec`（`:3305`）→ `chatgpt2api-green-ecbb260`（`:3304`）→ `chatgpt2api` `vps-20260614`（`:3303`）；live 容器在 `apps-interconnect` Docker 内网里有唯一别名 `img`，同网容器可用 `http://img` 内网访问；旧 FRP 回本机 `3003` 的链路保留为再下一级回滚路径，细节见 `llmdoc/guides/domain-and-frp.md`。
