...
- 当前项目还有一条仓库外入口 `img.fkcodex.com`。主线路已经迁到 VPS：远端宿主机 Nginx 当前反代 `127.0.0.1:3304`，live 容器是 `chatgpt2api-green-ecbb260`（镜像 `chatgpt2api:ecbb260`），旧 `chatgpt2api` 容器仍在 `127.0.0.1:3303` 作为回滚目标；旧 FRP 回本机 `3003` 的链路保留为二级回滚路径，细节见 `llmdoc/guides/domain-and-frp.md`。
