...
- 当前项目还有一条仓库外入口 `img.fkcodex.com`。主线路已经迁到 VPS：远端宿主机 Nginx 反代 `127.0.0.1:3303`，VPS 容器运行在 `/opt/chatgpt2api`，并加入 Docker 网络 `apps-interconnect`；旧 FRP 回本机 `3003` 的链路保留为回滚路径，细节见 `llmdoc/guides/domain-and-frp.md`。
