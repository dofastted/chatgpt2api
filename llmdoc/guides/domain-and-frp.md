...
- 远端服务器宿主机 Nginx 接收 HTTPS 请求，并反代到 `127.0.0.1:3307`（配置在 `/etc/nginx/sites-available/img.fkcodex.com`，切流量只改这一行 `proxy_pass` 端口后 `nginx -t && nginx -s reload`）。
- 当前 live 容器是 `chatgpt2api-green-94f19c7`，镜像是 `ghcr.io/dofastted/chatgpt2api:sha-94f19c7`，端口只绑定 `127.0.0.1:3307:80`。该版本包含账号导入后验活、失效凭据自动停用、瞬时错误保护、代理诊断和账号 Dashboard 重构。
- 回滚链（端口均保留、容器都在 `apps-interconnect`）：一级 `chatgpt2api-green-e8c98de`（`chatgpt2api:e8c98de`）在 `127.0.0.1:3306`；二级 `chatgpt2api-green-92129ec`（`chatgpt2api:92129ec`）在 `127.0.0.1:3305`；三级 `chatgpt2api-green-ecbb260`（`chatgpt2api:ecbb260`）在 `127.0.0.1:3304`；四级 `chatgpt2api`（`chatgpt2api:vps-20260614`）在 `127.0.0.1:3303`。
- 同一台 VPS 的 `apps-interconnect` Docker 内网里，live 容器 `chatgpt2api-green-94f19c7` 有网络别名 `img`；同网容器（例如 `newapi-app`）应使用 `http://img` 访问站点，不走公网，也不加 `:3003`。
...
- 首选回滚：把远端 Nginx upstream 从 `127.0.0.1:3307` 改回旧容器 `127.0.0.1:3306`，执行 `nginx -t && nginx -s reload`；旧容器和持久化数据挂载均保留。
...
- 2026-07-24 发布时发现本机 `frpc` 容器已退出，导致远端 `3208` 拒绝连接；已重新启动并确认 `restart=unless-stopped`。代理检测返回 `reachable=true`，VPS 容器通过 `http://172.20.0.1:3208` 访问 ChatGPT 出口。
