...
- 前端收到对应完成事件和 `[DONE]` 后，才能把会话状态从生成中改为完成。只收到图片内容但没有结束事件时，应继续视为协议错误，但如果该 turn 有 `queueRequestId`，页面会先查 `/api/image-queue/me?request_id=...`：查到活动请求则保留 pending 并后台恢复，查到 `finished + response_id` 则读取 `/v1/responses/{response_id}` 补全结果，查到 `failed/rejected` 才转错误态。
